...
#define MINUSSYM 276 
#define NUMBER 277
#define DIVSYM 278    /* Added division operator symbol */
#define MODSYM 279    /* Added modulus operator symbol */
...
"=" return ASGNSYM;			
"-" return MINUSSYM;	
"/" return DIVSYM;          /* Added division operator */
"%" return MODSYM;          /* Added modulus operator */		
{ident}     {
...
main()				/* Not needed with a parser */	
{
  ...
  do
  {
    ...
    else if (ch == DIVSYM)      /* Added division operator check */
      printf("%s\t\tDIV_OP ", yytext);
    else if (ch == MODSYM)      /* Added modulus operator check */
